...
test('se o topo da aplicação contém um conjunto fixo de links', () => {
  ...
  const FavoriteLinkElement = screen.getByRole('link', { name: /^Favorite Pokémons$/i });
...
test('se a aplicação é redirecionada para a página de About ao clicar no link About',
  () => {
    const { history } = renderWithRouter(<App />);
...
test(`se a aplicação é redirecionada para a página de Pokémons Favoritados ao 
